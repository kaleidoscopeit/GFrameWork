...
class root_htmlview
{
  ...
  function __flush(&$_)
  {
    ...
    /* static and dynamic webgets JS inclusions */
    foreach ($_->libraries as $library) {
      ...
      $lib_js_path  = $_->WEBGETS_PATH . implode('/js/', $library);

      /* includes dynamic js files */
      if(is_file($lib_js_path.'.js.php')) {
        require $lib_js_path . ".js.php";
      }
    }

    /* dynamic js of current view */
    $this->js_includes['?djs/' . $_->CALL_PATH] = true;
    ...
    switch($_->CALL_OBJECT){
      case 'views' :
       ...
       // $top_code[] = '<!DOCTYPE html4>';
        ...
        $top_code[] = '<link rel="stylesheet" type="text/css" '
                    . 'href="?css/webgets" />';
        $top_code[] = '<link rel="stylesheet" type="text/css" '
                    . 'href="?css/view/' . $_->CALL_PATH . '" />';

        /* JS Includes */
        $top_code[] = '<script type="text/JavaScript" '
                    . 'src="?js/webgets/htmlview"></script>';
        $top_code[] = '<script type="text/JavaScript" src="'
                    . '?js/view/' . $_->CALL_PATH . '"></script>';
      ...
      case 'subview' :
        ...
        $_->static[$_->CALL_URN]['css'] = $this->css_rules;
                  ...
                  $refer    = "parent" )
  {
    ...
    strpos($hsize, '%')   ? $wpc = str_replace("%", "", $hsize)
      : $wpx = str_replace("px", "", $hsize);
    strpos($vsize, '%')   ? $hpc = str_replace("%", "", $vsize)
      : $hpx = str_replace("px", "", $vsize);
    strpos($hoffset, '%') ? $lpc = str_replace("%", "", $hoffset)
      : $lpx = str_replace("px", "", $hoffset);
    strpos($voffset, '%') ? $tpc = str_replace("%", "", $voffset)
      : $tpx = str_replace("px", "", $voffset);

    if (strpos($halign, '%') || strpos($halign, 'px'))
      strpos($halign, '%') ? $rpc = str_replace("%", "", $halign)
        : $rpx = str_replace("px", "", $halign);

    if (strpos($valign, '%') || strpos($valign, 'px'))
      strpos($valign, '%') ? $bpc = str_replace("%", "", $valign)
        : $bpx = str_replace("px", "", $valign);

    $position['root']   = "position:fixed;";
    $position['parent'] = "position:absolute;";

    $hpos['left']   = $hoffset != 0 ? "left:" . $hoffset . ";" : null;
    $hpos['center'] = "left:" . @round(0+$lpx-$wpx/2,2) . "px;"
                    . "margin-left:" . round(50-$wpc/2+$lpc,2) . "%;";
    $hpos['right']  = "right:" . -($lpc + $lpx)
                    . ($lpx == "0" ? "%" : "px") . ";";

    $vpos['top']    = "top:" . $voffset . ";";
    $vpos['middle'] = "top:" . round(50-$hpc/2+$tpc,2) . "%;"
                    . "margin-top:" . @round(0-$hpx/2+$tpx,2) . "px;";
    $vpos['bottom'] = "bottom:" . -($tpc + $tpx)
                    . ($tpx == "0" ? "%" : "px") . ";";
    ...
    else $hparams = ($hsize != '' ? "width:" . $hsize . ";" : '')
                   . $hpos[$halign] . ';';
