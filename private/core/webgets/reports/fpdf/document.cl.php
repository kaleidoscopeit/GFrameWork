...
class reports_fpdf_document
{
  ...
  function __define(&$_)
  {
    ...
    /* istantiate the alias of the number of pages */
  ...
  function update_styles ()
  {
    ...
    if(isset($this->style['fill_color'][0])){
      ...
      if(isset($fill_color[2]))
        $this->fpdf->SetFillColor($fill_color[0],$fill_color[1],$fill_color[2]);

      else if(is_numeric($fill_color[0])) 
        $this->fpdf->SetFillColor($fill_color[0]);
    }
...
//    $this->fpdf->SetLineWidth(@$this->style['line_width'][0]);
  ...
  function set_local_style (&$webget, $styles = null)
  {
    ...
    foreach($inheritables as $style) {                                             // apply inherited style
      ...
      if(!isset($webget->$style)) $webget->$style = $this->style[$style][0];       // if the webget hasn't its own style sets the parent one
  ...
  function restore_style (&$webget, $styles = null)
  {
    $inheritables = array                                                         // defines common inheritable styles
    (
      'text_color',
    ...
    foreach($inheritables as $style) {
      ...
      if(!isset($webget->defaults[$style])) unset($webget->$style);               // if the webget hasn't its own default style unset (clean) the local grabbed
    }
