$_.js.reg['0130']={
  ...
  b:function(n){with(n){
    ...
    for(var x=0;x<cl;x++){$_.addClass(c[x], n.classfx);}
    ...
    n.show=function(i,x,c,f){with(n){
      ...
      if(n.selectedIndex!=null){
      	if(n.selectedIndex<i) $_.addClass(c[n.selectedIndex], n.classfxPrev);
      	if(n.selectedIndex>i) $_.addClass(c[n.selectedIndex], n.classfxNext);
      }

      try {
        $_.removeClass(c[i], n.classfxNext);
        $_.removeClass(c[i], n.classfxPrev);
      }
    ...
    n.trans_end=function(){
      ...
      for(x=0;x<cl;x++){
        if(c[x]!=selected){
         $_.removeClass(c[x], classfxNext);
         $_.removeClass(c[x], classfxPrev);
         if(selectedIndex<x)$_.addClass(c[x], classfxNext);
         else $_.addClass(c[x], classfxPrev);
