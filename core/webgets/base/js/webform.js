var $_={
 ...
 // grab webget
 gbw:function(l,w,sw,tw,cw){for(w in l){
    eval('if(!window.'+l[w]+')window.'+l[w]+'=this.gei(l[w]);')}},
 ...
 /* import in the javascript context ($_.webgets) all webgets and glues 
    them with the hierarchy relationship */
 hcy:function(h){
document.body.id='root';
  this.hcyiplus(document.body,{childWebgets:[]});
 ...
 hcyi:function(s,ch,po,co,w){
  ...
  hcyiplus:function(w,p){
    ...
    if(ch.length>0){
      w.childWebgets=[];
      $_.each(ch,function(v,i){ 
        $_.hcyiplus(v,(w.id?w:p));
      });
    }
 ...
 call:function(m,b,h) {  
  ...
  catch(e){alert('Parsing Call response failed : '+x.responseText);
    return false;}
 ...
 jsimport:function(l){
  ...
  catch(e){alert(xhr.responseText);er=1;
