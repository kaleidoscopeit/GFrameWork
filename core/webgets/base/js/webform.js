var $_ = {
  ...
  mloop : function(w, p) {
    ...
    if (id)
      eval('if(window.' + id + ' != w) window.' + id + '=w;');
    if (w.wid) {
  ...
  dde : function(obj, eve, hdl) {
    if (obj.detachEvent)
      obj.detachEvent("on" + eve, hdl);
    else
      obj.removeEventListener(eve, hdl, false);
  },
