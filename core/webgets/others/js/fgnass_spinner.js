$_.lib.pack_fgnass_spinner={
	...
	build:function(n){with(n){
		n.fs=function(s){n.show(n.preset|0)}
		...
		n.show=function(i,x,c,f){with(n){
			...
			selected.style.width="100%";selected.style.height="100%";
			
		
			// itherates through all child-nodes and hides all exept the selected in 'i'
			for(x=0;x<cl;x++){
				if(x!=i){
					c[x].style.width="0px";c[x].style.height="0px";
				}
			}
			
			// Executes panel's onshow code
			f=function(){eval(selected.getAttribute('onshow'))};			
			f.call(selected);
		}}
	}}
};
