...
class form_list {
    ...
    if ($this->nopaint) return;
		...
		$_->buffer .=	'<div wid="02B0" '.$boxing.'>'.
								'<select name="'.$this->id.'" id="'.$this->id.'" wid="02B1" multiple '.$css_style.
	...
	function items_insert($value, $label = -1, $index = null)
	{
		...
		$this->items['values'] =
  			array_merge (
     			($this->items['values'] ? array_slice ( $this->items['values'], 0, $index) : Array()),
     			Array("$value"),
     			($this->items['values'] ? array_slice ( $this->items['values'], $index) : Array())
     		);
     	$this->items['labels'] =
     		array_merge (
     			($this->items['labels'] ? array_slice ( $this->items['labels'], 0, $index) : Array()),
     			Array ( ( $label != -1 ? $label : $value ) ),
     			($this->items['labels'] ? array_slice ( $this->items['labels'], $index) : Array())
     		);
	}
