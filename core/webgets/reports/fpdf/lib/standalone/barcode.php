	...
	static public function raw($type, $datas) {
        ...
        switch($type){
            ...
            case 'code128':
                ...
                break;
            case 'code128noext':
                $digit = Barcode128::getDigit($code, true);
                $hri = $code;
                break;
            case 'codabar':
...
class Barcode128 {
    ...
    static public function getDigit($code, $noextended = false){
        ...
        // Extended ASCII code creation is enabled by default, but on request
        // can be disabled in order to let the use of subet applications like
        // GS1-128
        if( $noextended ) {
            for($i=0; $i<$len; $i++){
                if (strpos($tableB, $code[$i]) === false && ord($code[$i]) !== 207) return("");
        ...
        // check if only numeric character or FNC1 is present
        for($i=0; $i<3 && $i<$len; $i++){
            $tableCActivated &= preg_match('`[0-9 ' . chr(207) . ']`', $code[$i]);
        }
        ...
        while( $i < $len ){
            if (! $tableCActivated){
              ...
              || (preg_match('`[^0-9]`', $code[$i]) && !preg_match('`[' . chr(207) . ']`', $code[$i]))
              || (preg_match('`[^0-9]`', $code[$i+1])) ) { //todo : verifier le JS : len - 1!!! XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
                $tableCActivated = false;
                $result .= self::$encoding[ 100 ]; // B table
                $sum += ++$isum * 100;
            }

            if ( $tableCActivated && ord($code[$i]) !== 207) {
                $value = intval(substr($code, $i, 2)); // Add two characters (numeric)
                $i += 2;
            } else {
                // take care of extended characters (ASCII above 127)
                if (ord($code[$i])>126 && !$noextended) {
                    $result .= self::$encoding[ 100 ];
                    $sum += ++$isum * 100;
                    $value = strpos($tableB, chr(ord($code[$i])-128)); // Add one character
                } else {
                  if (ord($code[$i]) == 207) $value = 102;
                  else $value = strpos($tableB, $code[$i]); // Add one character
                }
                $i++;
            }
